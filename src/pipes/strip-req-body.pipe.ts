...
export class StripRequestBodyPipe implements PipeTransform {
  manualWhitelist<T extends object>(dtoClass: new () => T, plain: Record<string, unknown>): Partial<T> {
    const metadataStorage = getMetadataStorage();
    const metadata = metadataStorage.getTargetValidationMetadatas(dtoClass, '', false, false);
    ...
    for (const key of dtoKeys) {
      ...
      if (meta?.type === ValidationTypes.NESTED_VALIDATION && value && typeof value === 'object') {
        const nestedType = Reflect.getMetadata('design:type', dtoClass.prototype, key) as new () => any;

        if (Array.isArray(value)) {
          result[key] = value.map((item) => this.manualWhitelist(nestedType, item as Record<string, unknown>));
        } else {
          result[key] = this.manualWhitelist(nestedType, value as Record<string, unknown>);
        }
      } else {
        result[key] = value;
      }
